"""
Multi-Passenger Taxi — SARSA Training Script
"""
...
output_dir = Path("results/sarsa/multi_passenger")
output_dir.mkdir(parents=True, exist_ok=True)

checkpoints_dir = Path("results/sarsa/multi_passenger/checkpoints")
checkpoints_dir.mkdir(parents=True, exist_ok=True)

qtables_dir = Path("results/sarsa/multi_passenger/q_tables")
qtables_dir.mkdir(parents=True, exist_ok=True)

rewards_dir = Path("results/sarsa/multi_passenger/rewards")
...
def train_sarsa(
    ...
    """Train a SARSA agent with or without action masking.
    ...
    def select_action(state, info, current_epsilon):
        ...
        if np.random.random() < current_epsilon:
            ...
            if use_action_mask:
                ...
                if len(valid_actions) == 0:
                    return int(np.random.randint(0, n_actions))
                return int(valid_actions[np.argmax(q_table[state, valid_actions])])
    ...
    for episode in range(episodes):
        ...
        # SARSA starts by selecting an action
...
if __name__ == "__main__":
    ...
    plt.title(f"SARSA: Exploration Strategy & Masking - Multi-Passenger Taxi (n={N_PASSENGERS})")
    plt.legend()
    ...
    ax1.set_title(f"Effect of Decaying eps Schedule - SARSA Multi-Passenger (n={N_PASSENGERS})")
    ax1.legend()
