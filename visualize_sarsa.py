...
# Load SARSA Q-tables
all_qtables = np.load("results/sarsa/sarsa_masked_static_qtables.npy", allow_pickle=True)

output_dir = Path("results/sarsa/gifs")
...
for i in range(N_RUNS):
    ...
    record_episode(
        run_qtable,
        filename=output_dir / f"sarsa_masked_static_run_{i+1}.gif",
        seed=seed,
        epsilon=0.0,
    )
